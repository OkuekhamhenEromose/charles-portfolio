...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center justify-between h-14 rounded-2xl px-5
                      transition-all duration-300
                      ${
                        scrolled
                          ? "bg-card/80 backdrop-blur-xl border border-border shadow-lg shadow-background/30"
        ...
        >
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="nav-link px-3 py-1.5 text-sm font-medium rounded-lg hover:bg-accent/60 transition-all duration-200"
              >
                {item.name}
              </a>
            ))}
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <div className="xl:hidden socials-container relative">
              ...
              >
                <Share2
                  className={`w-4 h-4 transition-transform duration-300 ${socialsOpen ? "rotate-180" : ""}`}
                />
              </motion.button>
